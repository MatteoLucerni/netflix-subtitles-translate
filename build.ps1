<#
.SYNOPSIS
    Packages the Sublens Chrome extension for Chrome Web Store upload.

.DESCRIPTION
    Reads manifest.json to extract short_name and version, then creates a zip
    containing ONLY the extension files (no docs, .git, etc.).
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "sublens-build-$timestamp"
...
try {
    ...
    if ($envRelPath) {
        ...
        Set-Content -Path $envFile -Value 'self.DEV_MODE = false;' -Encoding UTF8
